...
try:
    while True:
        ...
        if results and len(results) > 0 and results[0].boxes is not None and len(results[0].boxes) > 0:
            ...
            if all_x and all_y:
                ...
                if (time.time() - last_send) >= COMMAND_DELAY:
                    ...
                    if cmd_count % 5 == 0:
                        print(f"[{cmd_count:3d}] Pan: {pan:3d}° | Tilt: {tilt:3d}° | Objects: {len(boxes)}")
        ...
        # Show status
        cv2.putText(frame, f"Commands: {cmd_count}", (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        cv2.putText(frame, "Both Pan & Tilt Working!", (10, 60),
